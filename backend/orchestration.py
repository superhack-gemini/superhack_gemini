...
def fanout_search_node(state: NarrativeState):
    ...
    """
    _log("🔍 PHASE 1: Research Planning")
    _log("Analyzing topic and generating search queries...")
    ...
    _log(f"✓ Generated {len(queries)} research queries")
...
def research_node(state: NarrativeState):
    ...
    """
    _log("🔎 PHASE 2: Executing Research")
    queries = state.get('research_results', {}).get('search_fanout_queries', [])
    
    if not queries:
        _log("⚠️ No queries to research", "warning")
        return {"current_phase": "research_failed", "error": "No queries to research"}
        
    context = []
    for i, q in enumerate(queries, 1):
        try:
            _log(f"  Searching ({i}/{len(queries)}): {q}")
            result = google_search.invoke(q)
            context.append(f"Query: {q}\nResult: {result}")
        except Exception as e:
            _log(f"  ⚠️ Search failed: {q}", "warning")
            
    combined_context = "\n\n".join(context)
    _log(f"✓ Research complete - gathered {len(context)} results")
...
def script_generation_node(state: NarrativeState):
    ...
    """
    _log("📝 PHASE 3: Script Generation")
    _log("Creating broadcast script with Gemini...")
    ...
    try:
        _log("  Invoking Gemini for structured script output...")
        ...
        num_segments = len(script_dict.get('segments', []))
        ai_segs = len([s for s in script_dict.get('segments', []) if s.get('type') == 'ai_generated'])
        clip_segs = num_segments - ai_segs
        
        _log(f"✓ Script generated: \"{script_dict.get('title', 'Untitled')}\"")
        _log(f"  Total segments: {num_segments} ({ai_segs} AI-generated, {clip_segs} real clips)")
        ...
        _log(f"❌ Script generation failed: {e}", "error")
...
async def process_veo_segments(script: Dict[str, Any]) -> Dict[str, List[Any]]:
    ...
    _log(f"🎥 Veo Generation: {len(ai_segments)} segments queued")
...
async def process_clip_workflow(script: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    _log(f"🎞️ Clip Retrieval: {len(clip_segments)} clips queued")
    ...
    async def process_single_clip(seg):
        ...
        try:
            ...
            # Upload
            _log(f"  🔬 Clip {order}: Analyzing content with Gemini...")
            ...
            _log(f"  ✂️ Clip {order}: Finding best {target_duration}s segment...")
            ...
            completed_count += 1
            _log(f"  ✓ Clip {order}: Ready ({completed_count}/{len(clip_segments)})")
            return result_clip
            
        except Exception as e:
            _log(f"  ❌ Clip {order}: Failed - {str(e)[:50]}", "error")
...
async def media_production_logic(state: NarrativeState) -> Dict[str, Any]:
    ...
    """
    _log("🎬 PHASE 4: Media Production (Parallel)")
    _log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    
    script = state.get('script')
    if not script:
        _log("❌ No script found for media production", "error")
        return {"error": "No script found"}
    
    segments = script.get('segments', [])
    ai_segs = [s for s in segments if s.get('type') == 'ai_generated']
    clip_segs = [s for s in segments if s.get('type') == 'real_clip']
    
    _log(f"  Starting parallel production:")
    _log(f"    • {len(ai_segs)} Veo AI videos to generate")
    _log(f"    • {len(clip_segs)} real clips to retrieve & cut")
    ...
    _log(f"✓ Media production complete:")
    _log(f"    • Veo videos: {len(veo_results['generated'])} succeeded, {len(veo_results['failed'])} failed")
    _log(f"    • Real clips: {len(clip_results)} retrieved")
...
def assembly_node(state: NarrativeState):
    ...
    if not all_clips:
        _log("❌ No clips to assemble", "error")
    ...
    try:
        ...
        _log(f"  Assembling {len(video_paths)} video segments...")
        for i, p in enumerate(video_paths, 1):
            _log(f"    {i}. {os.path.basename(p)}")

        if not video_paths:
            _log("❌ No valid video files found", "error")
        ...
        _log(f"  🔧 Concatenating with FFmpeg...")
        combine_videos(video_paths, output_path)
        _log(f"✓ Final video assembled: {output_filename}")
        ...
        _log(f"❌ Assembly failed: {e}", "error")
...
# Global log function placeholder - set by run_workflow
_log_fn = None

def _log(message: str, level: str = "info"):
    """Internal logging helper that uses the global log function if set."""
    if _log_fn:
        _log_fn(message, level)
    print(f"[{level.upper()}] {message}")


def run_workflow(prompt: str, duration_seconds: int = 150, log_fn=None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        log_fn: Optional logging function to report progress
    ...
    _log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    _log("🏈 SPORTS NARRATIVE GENERATOR")
    _log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
